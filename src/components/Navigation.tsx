...
function TabButton({ path, icon, label, longPressPath }: TabButtonProps) {
  ...
  return (
    ...
    >
      ...
      <div className="w-8 h-8 text-destructive flex items-center justify-center drop-shadow-[0_0_3px_hsl(var(--accent))]" aria-hidden="true">
        {icon}
      </div>
      <span className="text-xs font-bold text-destructive drop-shadow-[0_0_3px_hsl(var(--accent))]">{label}</span>
    </button>
  );
}
